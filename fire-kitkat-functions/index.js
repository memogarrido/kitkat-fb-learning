...
exports.createMap = functions.storage.object().onFinalize(object => {
  ...
  return client
    .faceDetection(`gs://${object.bucket}/${object.name}`)
    .then(results => {
      ...
      faces.forEach((face, i) => {
        ...
        var leftEyeDistanceValue = parseFloat(
          getDistanceRatio(leftEye, noseTip, reference)
        );
        var rightEyeDistanceValue = parseFloat(
          getDistanceRatio(noseTip, rightEye, reference)
        );
        var leftMouthDistanceValue = parseFloat(
          getDistanceRatio(noseTip, mouthLeft, reference)
        );
        var rightMouthDistanceValue = parseFloat(
          getDistanceRatio(noseTip, mouthRight, reference)
        );

        var data = {
          faceMap: {
            leftMouthDistance: leftMouthDistanceValue,
            rightEyeDistance: rightEyeDistanceValue,
            leftEyeDistance: leftEyeDistanceValue,
            rightMouthDistance: rightMouthDistanceValue
          }
        };
    ...
    .then(snapshot => {
      ...
      snapshot.forEach(doc => {
        ...
        var artis = doc.data();
         console.log(
          "facemap field",
          fcMap.rightEyeDistance,
          "keys"
        );

        var leftEyeDifference = Math.abs(
          100 - (leftEyeDistance * 100) / fcMap.leftEyeDistance
        );
        var rightEyeDifference = Math.abs(
          100 - (rightEyeDistance * 100) / fcMap.rightEyeDistance
        );
        var leftMouthDifference = Math.abs(
          100 - (leftMouthDistance * 100) / fcMap.leftMouthDistance
        );
        var rightMouthEyeDifference = Math.abs(
          100 - (rightMouthDistance * 100) / fcMap.rightMouthDistance
        );
        ...
        var differenceAverage =
          (leftEyeDifference +
            rightEyeDifference +
            leftMouthDifference +
            rightMouthEyeDifference) /
          4;
          artis.closeness =differenceAverage;
          artistList.push(artis);
